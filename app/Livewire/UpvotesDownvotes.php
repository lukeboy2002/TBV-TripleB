...
class UpvotesDownvotes extends Component
{
    ...
    public function render()
    {
        ...
        $user = request()->user();
        if ($user) {
            $model = UpvoteDownvote::where('post_id', '=', $this->post->id)->where('user_id', '=', $user->id)->first();
            if ($model) {
                $hasUpvote = !!$model->is_upvote;
            }
        }

        return view('livewire.upvotes-downvotes', compact('upvotes', 'downvotes', 'hasUpvote'));
    }
